...
(function(ext) {
  ...
  function processInput(inputData) {
    ...
    for (var i=0; i < inputData.length; i++) {	//i는 0부터 시작하지만, 결국적으로 1이 되서야  inputData[i] 를 storedInputData 에 담기 시작할 것임
      if (parsingSysex) {
		if ((inputData[0] == SCBD_CHOCOPI_USB || inputData[0] == SCBD_CHOCOPI_BLE) && pingCount < 6 && i === 11) { 
		  ...
		  //예상값) storedInputData[0] = 0xE0 혹은 0xF0
			...
			if (i < 11){		
				storedInputData[sysexBytesRead++] = inputData[i];				//detail/port + Data ( 10 Byte) = 11 Byte 이상이면 강제로 끊어버림 sysexBytesRead === 10
